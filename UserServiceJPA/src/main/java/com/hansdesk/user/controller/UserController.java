...
public class UserController {
    ...
    public String postSignUp(@ModelAttribute("signUpForm") @Valid SignUpForm signUpForm, BindingResult bindingResult, Model model, RedirectAttributes redirectAttributes) {
        ...
        if (!userService.isUniqueEmail(signUpForm.getEmail()))
            bindingResult.rejectValue("email", "Unique", "The value is already used.");
        if (!userService.isUniqueNickname(signUpForm.getNickname()))
            bindingResult.rejectValue("nickname", "Unique", "The value is already used.");
    ...
    public String postSignUpNext(@ModelAttribute("signUpNextForm") @Valid SignUpNextForm signUpNextForm, BindingResult bindingResult, Model model) {
        SignUpUser signUpUser = userService.getSignUpUser(signUpNextForm.getEmail());
        if (signUpUser == null)
        ...
        if (!userService.registerUser(signUpUser, signUpNextForm, Role.ROLE_USER))
